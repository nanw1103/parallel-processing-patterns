...
**Operation cascading** is a pattern that a downstream operation is triggered directly upon the upstream operation. There are many forms of how it exists. For example, on condition A, do operation B, and inside operation B, do operation C. Operation C is a cascaded operation from operation A. Operation cascading has advantages and suitable scenarios due to its simplicity and intuition. But in a complex system, especially subsystems with different processing paces, the simple operation cascading could lead to multiple issues. One outstanding issue is that the entire system throughput could be impacted by the slowest point, and normally hard to scale properly.

On the contrary, another approach is **system decoupling**, which breaks the system into multiple _stages_, so each stage can be designed and optimized with proper scale target. This will lead to a more complex design and implementation, compared to operation cascading. Related patterns are producer-consumer, _batch stage chain_, and mark-and-sweep.
